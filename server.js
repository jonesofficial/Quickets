...
// ---------- City alias map (common short forms) ----------
const cityAliasMap = {
  // southern
  "hyd": "Hyderabad",
  "hydr": "Hyderabad",
...
// normalize a city token for alias lookup
function normTokenCity(s) {
  if (!s) return "";
  return String(s).toLowerCase().replace(/[^a-z0-9]/g, ""); // remove dots/spaces
}

// resolve if input is a known alias or exact normalized match to canonical list
function resolveCityAlias(input) {
  if (!input) return null;
  const q = String(input).trim();
  const key = normTokenCity(q);

  // alias hit
  if (cityAliasMap[key]) return { kind: "alias", canonical: cityAliasMap[key], raw: q };

  // exact match on normalized city name: e.g., "newdelhi" => "New Delhi"
  if (/^[A-Za-z ]+$/.test(q) && q.replace(/\s+/g, "").length >= 3) {
    const cand = q.split(/\s+/).map(w => w[0]?.toUpperCase() + w.slice(1).toLowerCase()).join(" ");
    return { kind: "exact", canonical: cand, raw: q };
  }
...
app.post("/webhook", async (req, res) => {
  try {
    ...
    // --- Confirm-from / Confirm-to button handlers (must come early) ---
    if (interactiveType === "button_reply") {
      // CONFIRM_BOARDING buttons
    ...
    if (s.state === "BUS_FROM" && msg.type === "text") {
      ...
      if (resolved && resolved.kind === "alias") {
        // ask to confirm alias expansion using buttons
        s.__pendingFromCandidate = resolved.canonical; // store canonical for later
        s.state = "CONFIRM_BOARDING";
        await sendButtons(from, `Do you mean *${resolved.canonical}* for "${candidate}"?`, [
      ...
      // exact candidate (no ambiguity) -> accept and continue
      s.pendingBooking.from = resolved ? resolved.canonical : candidate;
      s.state = "BUS_TO";
      await askBusTo(from);
      return res.sendStatus(200);
    }
    ...
    if (s.state === "BUS_TO" && msg.type === "text") {
      ...
      s.pendingBooking.to = resolved ? resolved.canonical : candidate;
      s.state = "BUS_DATE";
      await askBusDate(from);
      return res.sendStatus(200);
    }
