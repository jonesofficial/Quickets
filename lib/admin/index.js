...
async function handleAdminCommands(ctx) {
  try {
    ...
    /* ==============================
   PDF Upload → Tickets (BAILEYS SAFE)
============================== */

    const documentMessage =
      ctx.msg?.message?.documentMessage ||
      ctx.msg?.document ||
      ctx.msg?.documentMessage;

    if (documentMessage) {
      const mime = documentMessage.mimetype || "";

      console.log("📄 DOCUMENT RECEIVED MIME:", mime);

      // If mimetype missing, fallback by filename
    ...
    if (/^PAYMENT/i.test(upper)) return await handlePaymentCommands(ctx, text);
