...
async function handleAdminCommands(ctx) {
  try {
    ...
    /* =====================================================
   📄 PDF Upload → Ticket Mode (BAILEYS FINAL PRO)
===================================================== */
    ...
    const getDocumentMessage = (msg) => {
      ...
      if (msg.ephemeralMessage?.message) {
        return getDocumentMessage(msg.ephemeralMessage.message);
      }

      if (msg.viewOnceMessage?.message) {
        return getDocumentMessage(msg.viewOnceMessage.message);
      }

      return null;
    };

    const documentMessage = getDocumentMessage(ctx.msg?.message);

    if (documentMessage) {
      console.log("📄 DOCUMENT FOUND INNER");

      console.log("📄 FULL DOC OBJECT:");
      console.log(JSON.stringify(documentMessage, null, 2));
    ...
    const text =
      ctx.msg?.text?.body?.trim() || ctx.msg?.image?.caption?.trim() || "";
