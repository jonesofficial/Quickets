...
🆔 ${ctx.session.bookingId}`,
  ...
  if (command === "PROCESS") {
    ...
    switch (booking.type) {
      case "TRAIN":
        userPhone = booking.user; // train flow stores user number here
        break;

      case "BUS":
        userPhone = booking.from; // bus flow uses sender
        break;

      default:
        userPhone = booking.phone; // fallback for future domains
    }
