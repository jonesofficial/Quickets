...
• AVAILABLE ${bookingId}
• WAITING LIST ${bookingId}
• RAC ${bookingId}
• NO CHANCE ${bookingId}`,
        );

        return true; // 🔥 IMPORTANT: stop execution here
      }
...
We'll update you shortly.`,
    ...
    if (booking.type === "TRAIN" && !booking.quickMode) {
      try {
        const trains = await trainsBetween(booking.fromCode, booking.toCode);
