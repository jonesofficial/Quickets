...
const sendTrainFare = require("../../flow/domains/train/manual/fareFlow.js");
...
===================================================== */
  ...
  const mode = booking.type?.toUpperCase();
...
===================================================== */
  ...
  if (mode === "BUS") {
    ...
    const total = Number((data.base + data.gst + data.agent).toFixed(2));
...
💰 TOTAL  : ₹${total}`,
    ...
    const success = await sendBusFare({ booking: updatedBooking, ctx });

    if (!success) {
      await sendText(from, "⚠️ Fare stored but failed to send to user.");
      return true;
    }
...
===================================================== */
  ...
  if (mode === "TRAIN") {
    ...
    const total = Number((data.base + data.pg).toFixed(2));
...
💰 TOTAL: ₹${total}`,
    ...
    const success = await sendTrainFare({ booking: updatedBooking, ctx });
