...
========================= */
  ...
  try {
    const res = await pool.query(
      "INSERT INTO bookings (id,data) VALUES ($1,$2) ON CONFLICT (id) DO UPDATE SET data=$2 RETURNING id",
    ...
    if (res?.rows?.length) {
      ...
      console.warn("⚠️ Neon save returned no rows:", normalized.id);
    }
  } catch (err) {
...
========================= */
  ...
  try {
    ...
    if (res?.rows?.length) {
      ...
      console.warn("⚠️ Neon update returned no rows:", updated.id);
    }
  } catch (err) {
