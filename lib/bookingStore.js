...
function generateReadableBookingId(type = "UNKNOWN") {
  const prefixMap = {
    BUS: "QB",
    TRAIN: "QT",
    FLIGHT: "QF",
  ...
  const prefix = prefixMap[type] || "QX";
...
async function saveBooking(booking) {
  ...
  /* Ensure readable booking ID */
  if (
    normalized.status === "PROCESSING" &&
    normalized.id.startsWith("QK-")
  ) {
  ...
  try {
    ...
    if (res?.rows?.length) {
      console.log("✅ Neon save success:", res.rows[0].id);
    } else {
      console.warn(
        "⚠️ Neon save returned no rows:",
        normalized.id
      );
    }
  } catch (err) {
...
async function updateBooking(id, patch) {
  ...
  try {
    ...
    if (res?.rows?.length) {
      console.log("✅ Neon update success:", res.rows[0].id);
    }
  } catch (err) {
  ...
  if (
    ["CANCELLED", "FAILED", "PAYMENT_FAILED"].includes(
      updated.status
    )
  ) {
    removeInProcessBooking(updated.id);
...
function findBookingById(id) {
  ...
  return (
    all.find(
      (b) => String(b.id).trim().toUpperCase() === cleanId
    ) || null
  );
}
...
//   /* =================================
//      Prevent broken PAYMENT_PENDING
//   ================================= */

//   if (
//     updated.status === "PAYMENT_PENDING" &&
//     !updated.payment?.link
//   ) {
//     console.warn(
//       "⚠️ Prevented PAYMENT_PENDING without payment link:",
//       updated.id
//     );
