...
function markPaymentSuccess(booking) {
  ...
  // 🔥 FIX: Normalize missing fields
  if (!payment.status) {
    console.log("⚠️ Payment status missing → defaulting to CREATED");
    payment.status = "CREATED";
  }

  if (!payment.timestamps) {
    payment.timestamps = {};
  }

  const currentStatus = payment.status;

  // ✅ Allow real-world flexible transitions
  const allowedStatuses = ["PENDING", "CREATED", "FARE_SENT", "SUCCESS"];
  ...
  if (!allowedStatuses.includes(currentStatus)) {
    ...
    console.log(
      `⚠️ Allowing SUCCESS transition from ${currentStatus} (fallback)`
    );
  }
