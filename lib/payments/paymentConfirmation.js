...
async function handleScreenshot(ctx, booking) {
  try {
    ...
    // 🔥 SIMPLE IMAGE CHECK (your wrapper)
    if (ctx.msg?.type !== "image" || !ctx.msg?.image) {
      return false;
    }
...
— *Team Quickets*`
    ...
    /* ===============================
       Send SAME image to Admin
    =============================== */

    const ADMIN = process.env.ADMIN_PHONE || process.env.ADMIN_NUMBER;

    if (ADMIN && ctx.sendImage) {
      await ctx.sendImage(
        ADMIN,
        ctx.msg.image, // pass original image object
