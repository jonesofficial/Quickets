...
async function handleScreenshot(ctx, booking) {
  ...
  // Only accept screenshot in PAYMENT_PENDING stage
  if (booking.status !== "PAYMENT_PENDING") return false;

  // Prevent duplicate screenshot processing
  if (booking.payment?.screenshotReceived) return false;
...
async function handleAdminPaymentReceived(from, bookingId) {
  ...
  // Must have screenshot OR UTR before confirming
  if (
    !booking.payment?.screenshotReceived &&
    !booking.payment?.utr
  ) {
