...
async function handleDebug(ctx) {
  ...
  if (action === "PAYMENT") {
    ...
    try {
      /* ===== CREATE ===== */
      if (sub === "CREATE") {
        const fare = booking.fare || ctx.session.lockedFare || ctx.session.fare;
        ...
        booking.payment = createPayment({
          ...
          amount: {
            ...
            taxes: fare.gst,
            fee: fare.agent,
            total: fare.total || fare.base + fare.gst + fare.agent,
          },
        });
      ...
      if (sub === "AUTO") {
        ...
        await sendText(ctx.from, "✅ Auto payment created & initiated.");
        return true;
      }
  ...
  ========================================= */
  if (action === "PDF") {
    const hasPdf = !!ctx.session.pendingTicketPdf;

    const size = hasPdf ? Buffer.byteLength(ctx.session.pendingTicketPdf) : 0;

    const booking = ctx.session.bookingId
      ? findBookingById(ctx.session.bookingId)
      : getLastBookingByUser(ctx.from);

    let ticketInfo = "No booking found.";

    if (booking) {
      ticketInfo = booking.ticketFileName
        ? `Saved File: ${booking.ticketFileName}`
        : "No ticket file saved yet.";
    }

    await sendText(
      ctx.from,
      "📄 PDF DEBUG STATUS\n\n" +
        `In Session: ${hasPdf ? "✅ YES" : "❌ NO"}\n` +
        `Buffer Size: ${size} bytes\n\n` +
        `Booking: ${booking?.bookingId || "None"}\n` +
        `${ticketInfo}`,
    );
