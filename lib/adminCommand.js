...
const ADMIN_JID = RAW_ADMIN
  ? RAW_ADMIN.replace(/\D/g, "") + "@c.us"
  : null;
...
if (!ADMIN_JID || from !== ADMIN_JID) {
  ...
  // 🛡️ Only TEXT messages allowed for admin
  const text = ctx.msg?.text?.body?.trim();
  ...
  if (PAYMENT_COMMANDS[command]) {
    ...
    if (command === "PAYSUCCESS") patch.status = "CONFIRMED";
    if (command !== "PAYSUCCESS") patch.status = "PAYMENT_FAILED";
  }
