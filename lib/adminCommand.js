...
async function handleAdminCommands(ctx) {
  ...
  if (BOOKING_COMMANDS[command]) {
    ...
    if (command === "FAIL" && booking.user) {
      await sendText(
        booking.user,
        `❌ *Booking Update*\n\n` +
          `Unfortunately, we were unable to process your booking.\n\n` +
          `${reason ? `Reason: ${reason}\n\n` : ""}` +
          `You may try again anytime.\n\n` +
          `— *Team Quickets*`
      );
    }
  ...
  if (PAYMENT_COMMANDS[command]) {
    ...
    if (command === "PAYFAIL" && booking.user) {
      await sendText(
        booking.user,
        `⚠️ *Payment Failed*\n\n` +
          `${reason ? `Reason: ${reason}\n\n` : ""}` +
    ...
    if (command === "PAYCANCEL" && booking.user) {
      await sendText(
        booking.user,
        `🚫 *Payment Cancelled*\n\n` +
          `If you still wish to confirm your booking, please try again.\n\n` +
          `— *Team Quickets*`
      );
    }
