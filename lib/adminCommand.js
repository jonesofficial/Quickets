...
//     const image = ctx.session.lastSeatImage;
...
// key: admin phone (normalized)
// value: media_id or image link
...
async function handleAdminCommands(ctx) {
  ...
  if (ctx.msg?.image) {
    const image =
      ctx.msg.image.media_id || ctx.msg.image.link;

    if (image) {
      adminSeatImages.set(adminKey, image);
      hasImage = true;
      console.log("🖼️ Seat layout image stored (admin cache)");
    }
  }

  // 🔒 Reject non-admins
  if (!RAW_ADMIN || adminKey !== normalize(RAW_ADMIN)) {
    return false;
  }
  ...
  if (/^SEAT_OPTIONS/i.test(upper)) {
    ...
    // 🔑 Fetch image from admin cache (NOT session)
    const image = adminSeatImages.get(adminKey);
    ...
    // 🧹 Clear image after use (prevents reuse)
    adminSeatImages.delete(adminKey);
