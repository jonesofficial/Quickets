...
const trainAdminCommand = require(
  "./flow/domains/train/manual/trainAdminCommand"
);
//'''
/* ======================================================
 * ADMIN CONFIG
...
async function handleAdminCommands(ctx) {
  ...
  // 🔑 ACTIVE TYPE (CRITICAL FIX)
  const activeType = ctx.session?.pendingBooking?.type || null;

  /* ======================================================
   * 🪑 BUS SEAT OPTIONS (IMAGE BASED)
   * ====================================================== */
  if (/^SEAT[_\s]?OPTIONS/i.test(upper)) {
    if (activeType !== "BUS") {
      await sendText(
        from,
        "⚠️ *Seat options are only applicable for BUS bookings.*"
      );
      return true;
    }
  ...
  /* ======================================================
   * 🚆 TRAIN MANUAL FLOW (EARLY INTERCEPT)
   * ====================================================== */
  if (/^(AVAILABLE|WAITING\s+LIST)/i.test(upper)) {
    if (activeType && activeType !== "TRAIN") {
      await sendText(
        from,
  ...
  /* ======================================================
   * GENERIC BOOKING / PAYMENT COMMANDS
   * ====================================================== */
