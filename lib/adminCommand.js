...
//   if (ctx.msg?.image) {
//   ctx.session.lastSeatImage = ctx.msg.image.id || ctx.msg.image.link;
...
//     // Send seat layout image + instructions to user
//     // (image is expected to be attached in the admin message)
...
//     // 🚫 Prevent premature CONFIRM for BUS
...
//     // 🔑 Clear BUS session after payment
//     if (booking.type === "BUS") {
...
async function handleAdminCommands(ctx) {
   ...
   * ========================= */
  if (ctx.msg?.image) {
    ctx.session.lastSeatImage =
      ctx.msg.image.id || ctx.msg.image.link;
