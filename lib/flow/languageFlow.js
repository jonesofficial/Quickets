...
module.exports = async function languageFlow(ctx) {
  ...
  const id = (interactiveId || "").toLowerCase();

  console.log("LANG FLOW:", {
    state: s.state,
    interactiveType,
    interactiveId,
  });
  ...
  if (s.state === "LANG_SELECTION") {
    ...
    if (interactiveType === "button_reply") {
      ...
      if (id === "lang_en" || id === "english") {
        s.optionSet = "en";
      }

      else if (id === "lang_ta" || id === "tamil" || id === "தமிழ்") {
        s.optionSet = "ta";
      }

      else if (id === "lang_more") {
      ...
      /* 🚫 Prevent other flows from touching language clicks */
      else {
        return true;
      }
    }
    ...
    if (msg.type === "text") {
      ...
      else if (["2", "ta", "tamil", "தமிழ்"].includes(lower)) {
        s.optionSet = "ta";
      }

      else if (["3", "hi", "hindi", "हिन्दी", "हिंदी"].includes(lower)) {
        s.optionSet = "hi";
      }
    }
...
*What would you like to do next?*`,
    ...
    /* 🚫 Stop message from leaking to other flows */
    return true;
  }
