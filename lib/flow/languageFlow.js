...
module.exports = async function languageFlow(ctx) {
  ...
  if (!s.optionSet && s.state !== "LANG_SELECTION") {
    ...
    /* Wait 3 seconds before showing language buttons */
    await delay(3000);
  ...
  if (s.state === "LANG_SELECTION") {
    ...
    if (msg.type === "text") {
      ...
      if (["3", "hin", "hindi", "हिन्दी", "हिंदी"].includes(lower)) {
        s.optionSet = "hi";
        s.lang = "hi";
      }
    }
    ...
    if (s.optionSet) {
      ...
      await sendList(
        from,
        `Quickets helps you book Bus 🚌, Train 🚆, and Flight ✈️ tickets directly on WhatsApp.
