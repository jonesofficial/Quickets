...
module.exports = async function paymentFlow(ctx) {
   ...
   * PAY WITH UPI
   * ========================= */
  await sendButtons(from, `💳 *Pay using UPI*\n\nAmount: ₹${total}`, [
    {
      id: "IGNORE", // not used for URL buttons
      title: "💰 Pay using any UPI",
      url: booking.payment.link,
    },
  ]);
  ...
  if (buttonId === "PAY_QR" || text === "QR") {
    try {
      ...
      await sendImage(from, qr, `📷 *Scan to Pay*\n\nAmount: ₹${total}`);
      return true;
    } catch (err) {
