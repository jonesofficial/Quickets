...
module.exports = async function passengerFlow(ctx) {
  ...
  if (s.state === "BUS_SUMMARY" && interactiveType === "button_reply") {
    if (interactiveId === "CONFIRM_BOOK") {
      ...
      // 🎟️ Ticket-style confirmation (ADMIN)
      const ticketLines = [
        "🎟️ *QUIKETS — BOOKING CONFIRMED*",
        "",
        "━━━━━━━━━━━━━━━━━━",
        `🆔 Booking ID: ${b.id}`,
        `📱 Customer: ${from}`, // FULL sender number
        `📅 Booked On: ${bookedAt}`,
        "━━━━━━━━━━━━━━━━━━",
        "",
        "🚍 *Journey*",
        ...
        "ℹ️ *Next Steps*",
        "• Assign operator",
        "• Share final ticket with customer",
...
async function showSummary(ctx) {
  ...
  const lines = [
    ...
    `Customer No: ${from}`, // FULL sender number
