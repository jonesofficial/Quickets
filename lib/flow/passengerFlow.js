...
async function resendPassengerPrompt(ctx) {
  ...
  switch (s.state) {
    ...
    case "PAYMENT_PENDING":
      // 🔥 resend ONLY buttons (no text)
      return sendButtons(from, "Other options 👇", [
      ...
      if (interactiveId === "CONFIRM_BOOK") {
        ...
        await sendText(
          ...
          "✅ *Booking confirmed*\n\n" +
            ...
            "Use buttons below for Card / Net Banking."
        );

        // 2️⃣ QR (media)
        await sendPaymentQR(ctx);

        // 3️⃣ BUTTONS — MUST BE LAST
