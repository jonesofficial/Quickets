...
module.exports = async function passengerFlow(ctx) {
  ...
  // 🔒 Guard: only passenger-related states
  if (!s || !s.state || !s.state.startsWith("BUS_PAX") && s.state !== "BUS_SUMMARY") {
    return false;
  }
  ...
  if (s.state === "BUS_PAX_ONE_GENDER_WAIT" && interactiveType === "button_reply") {
    ...
    if (s.pendingBooking.passengers.length < total) {
      ...
      await sendText(
        from,
        get("ENTER_NAME_PROMPT", { i: s.__oneIndex, total })
      );
      return true;
    }
  ...
  /* ==================================================
   * SUMMARY ACTIONS (CONFIRM / EDIT / CANCEL)
   * ================================================== */
  if (s.state === "BUS_SUMMARY" && interactiveType === "button_reply") {
    console.log("PASSENGER FLOW:", s.state, interactiveType, interactiveId);
