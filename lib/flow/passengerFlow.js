...
module.exports = async function passengerFlow(ctx) {
  ...
  if (s.state === "BUS_CONFIRM" && interactiveType === "button_reply") {
    if (interactiveId === "CONFIRM_BOOK") {
      ...
      console.log("🧪 ADMIN_PHONE runtime value:", process.env.ADMIN_PHONE);
      ...
      // ✅ Try sending summary to admin (policy-safe)
      if (!process.env.ADMIN_PHONE) {
        console.error("❌ ADMIN_PHONE not set");
      } else {
        try {
          console.log("📤 Sending admin summary to:", process.env.ADMIN_PHONE);
          await sendText(process.env.ADMIN_PHONE, summary);
          console.log("✅ Admin summary sent");
        } catch (err) {
          console.error("❌ Admin send failed:", err);

          // Graceful fallback (no tech jargon)
          await sendText(
            from,
            "⚠️ Booking received successfully.\nOur team will contact you shortly."
          );
        }
      }
