// const { sendText, sendButtons } = require("../../../../waClient");
// const {
//   updateBooking,
//   findBookingById,
// } = require("../../../../bookingStore");
// const { startOrGet } = require("../../../../sessionStore");

// const TRAIN_STATES = require("./states");

// const RAW_ADMIN =
//   process.env.ADMIN_PHONE || process.env.ADMIN_NUMBER;

// /* ==========================================
//    CONSTANT CHARGES
// ========================================== */
// const IRCTC_FEE = 17.7;
// const AGENT_FEE = 20;

// /* ==========================================
//    FORMAT INR
// ========================================== */
// function formatINR(amount) {
//   return Number(amount).toLocaleString("en-IN", {
//     minimumFractionDigits: 2,
//     maximumFractionDigits: 2,
//   });
// }

// /* ==========================================
//    SEND TRAIN FARE
// ========================================== */
// module.exports = async function sendFare({ booking, ctx }) {
//   try {
//     if (!booking) {
//       await sendText(ctx?.from, "⚠️ No active booking found.");
//       return false;
//     }

//     const freshBooking = findBookingById(booking.id);

//     if (!freshBooking || !freshBooking.user) {
//       await sendText(ctx?.from, "⚠️ Booking not found.");
//       return false;
//     }

//     const fare = freshBooking.fare;

//     if (!fare || fare.base == null || fare.pg == null) {
//       await sendText(
//         freshBooking.user,
//         "❌ Fare details unavailable. Please wait..."
//       );
//       return false;
//     }

//     const ticketCost = Number(fare.base);
//     const pgCharge = Number(fare.pg);

//     if ([ticketCost, pgCharge].some(isNaN)) {
//       await sendText(
//         freshBooking.user,
//         "❌ Invalid fare received. Please contact support."
//       );
//       return false;
//     }

//     const irctcFee = IRCTC_FEE;
//     const agentFee = AGENT_FEE;

//     const total = Number(
//       (ticketCost + irctcFee + agentFee + pgCharge).toFixed(2)
//     );

//     /* ==========================================
//        UPDATE BOOKING + LOCK FARE
//     =========================================== */
//     updateBooking(freshBooking.id, {
//       fare: {
//         ticketCost,
//         irctcFee,
//         agentFee,
//         pgCharge,
//         total,
//         currency: "INR",
//         source: "IRCTC_MANUAL",
//         lockedAt: Date.now(),
//       },
//       status: "FARE_SENT",
//     });

//     /* ==========================================
//        UPDATE USER SESSION STATE
//     =========================================== */
//     const { session: userSession } = startOrGet(
//       freshBooking.user
//     );

//     userSession.state = TRAIN_STATES.FARE_SENT;
//     userSession.bookingId = freshBooking.id;

//     /* ==========================================
//        SEND FARE TO USER
//     =========================================== */
//     await sendButtons(
//       freshBooking.user,
//       `
// 🚆 *Final Train Ticket Price*

// 🎫 Train Fare      : ₹${formatINR(ticketCost)}
// 🏛 IRCTC Fee       : ₹${formatINR(irctcFee)}
// 💳 PG Charges      : ₹${formatINR(pgCharge)}
// 💼 Agent Fee       : ₹${formatINR(agentFee)}

// ━━━━━━━━━━━━━━━━━━
// 💰 *Total Payable* : ₹${formatINR(total)}
// ━━━━━━━━━━━━━━━━━━

// Please confirm to proceed with payment.
//       `.trim(),
//       [
//         { id: "PROCEED_PAYMENT", title: "💳 Proceed" },
//         { id: "CANCEL_BOOKING", title: "❌ Cancel" },
//       ]
//     );

//     /* ==========================================
//        NOTIFY ADMIN
//     =========================================== */
//     if (RAW_ADMIN) {
//       await sendText(
//         RAW_ADMIN,
//         `💰 Train Fare Sent

// 🆔 ${freshBooking.id}
// 👤 ${freshBooking.user}

// 💰 Total: ₹${formatINR(total)}

// Waiting for payment confirmation.`
//       );
//     }

//     return true;
//   } catch (err) {
//     console.error("🔥 FATAL TRAIN sendFare Error:", {
//       bookingId: booking?.id,
//       error: err.message,
//     });

//     if (booking?.user) {
//       await sendText(
//         booking.user,
//         "⚠️ Something went wrong while processing fare."
//       );
//     }

//     return false;
//   }
// };
const { sendText, sendButtons } = require("../../../../waClient");
const { updateBooking, findBookingById } = require("../../../../bookingStore");
const { startOrGet } = require("../../../../sessionStore");
const BOOKING_STATUS = require("../bookingStates");

const TRAIN_STATES = require("./states");
const { buildUPILink } = require("../../../../payments/paymentLinks"); // ✅ ADDED

const RAW_ADMIN = process.env.ADMIN_PHONE || process.env.ADMIN_NUMBER;

/* ==========================================
   CONSTANT CHARGES
========================================== */
const IRCTC_FEE = 17.7;
const AGENT_FEE = 20;

/* ==========================================
   FORMAT INR
========================================== */
function formatINR(amount) {
  return Number(amount).toLocaleString("en-IN", {
    minimumFractionDigits: 2,
    maximumFractionDigits: 2,
  });
}

/* ==========================================
   RESOLVE USER PHONE
========================================== */
function getUserPhone(booking) {
  if (!booking) return null;

  if (booking.quickMode) return booking.user;

  return booking.phone || booking.user;
}

/* ==========================================
   SEND TRAIN FARE
========================================== */
module.exports = async function sendFare({ booking, ctx }) {
  try {
    if (!booking) {
      await sendText(ctx?.from, "⚠️ No active booking found.");
      return false;
    }

    const freshBooking = findBookingById(booking.id);

    if (!freshBooking) {
      await sendText(ctx?.from, "⚠️ Booking not found.");
      return false;
    }

    if (!freshBooking.id) {
      console.log("❌ Invalid booking in sendFare");
      return false;
    }

    const userPhone = getUserPhone(freshBooking);

    if (!userPhone) {
      await sendText(ctx?.from, "⚠️ User phone not found.");
      return false;
    }

    const fare = freshBooking.fare;

    if (!fare || fare.base == null || fare.pg == null) {
      await sendText(userPhone, "❌ Fare details unavailable. Please wait...");
      return false;
    }

    const ticketCost = Number(fare.base);
    const pgCharge = Number(fare.pg);

    if ([ticketCost, pgCharge].some(isNaN)) {
      await sendText(
        userPhone,
        "❌ Invalid fare received. Please contact support.",
      );
      return false;
    }

    const irctcFee = IRCTC_FEE;
    const agentFee = AGENT_FEE;

    const total = Number(
      (ticketCost + irctcFee + agentFee + pgCharge).toFixed(2),
    );

    if (freshBooking.status === BOOKING_STATUS.FARE_SENT) {
      console.log("⚠️ Fare already sent, skipping duplicate");
      return true;
    }
    /* ==========================================
       CREATE PAYMENT LINK ✅
    =========================================== */
    const paymentLink = buildUPILink({
      upiId: process.env.UPI_ID || "yourupi@bank",
      name: "Quickets",
      amount: total,
      note: `Booking ${freshBooking.id}`,
    });

    /* ==========================================
       UPDATE BOOKING + PAYMENT INIT ✅
    =========================================== */

    console.log("💰 Sending fare:", {
      bookingId: freshBooking.id,
      total,
      user: userPhone,
    });

    updateBooking(freshBooking.id, {
      fare: {
        ticketCost,
        irctcFee,
        agentFee,
        pgCharge,
        total,
        currency: "INR",
        source: "IRCTC_MANUAL",
        lockedAt: Date.now(),
      },

      payment: {
        link: paymentLink,
        status: "CREATED",
        amount: {
          total,
          currency: "INR",
        },
        attempts: 0, // ✅ add
        timestamps: {
          createdAt: Date.now(), // ✅ move here
          paidAt: null,
          failedAt: null,
        },
      },

      status: BOOKING_STATUS.FARE_SENT,
    });

    /* ==========================================
       UPDATE USER SESSION
    =========================================== */
    const { session: userSession } = startOrGet(userPhone);

    userSession.state = TRAIN_STATES.FARE_SENT;
    userSession.bookingId = freshBooking.id;

    /* ==========================================
       SEND FARE TO USER
    =========================================== */
    await sendButtons(
      userPhone,
      `
🚆 *Final Train Ticket Price*

🎫 Train Fare      : ₹${formatINR(ticketCost)}
🏛 IRCTC Fee       : ₹${formatINR(irctcFee)}
💳 PG Charges      : ₹${formatINR(pgCharge)}
💼 Agent Fee       : ₹${formatINR(agentFee)}

━━━━━━━━━━━━━━━━━━
💰 *Total Payable* : ₹${formatINR(total)}
━━━━━━━━━━━━━━━━━━
👉 Pay here:
${paymentLink}


Please confirm to proceed with payment.
      `.trim(),
      [
        { id: "PROCEED_PAYMENT", title: "💳 Proceed" },
        { id: "CANCEL_BOOKING", title: "❌ Cancel" },
      ],
    );

    /* ==========================================
       ADMIN NOTIFY
    =========================================== */
    if (RAW_ADMIN) {
      await sendText(
        RAW_ADMIN,
        `💰 Train Fare Sent

🆔 ${freshBooking.id}
👤 ${userPhone}

💰 Total: ₹${formatINR(total)}

Waiting for payment confirmation.`,
      );
    }

    return true;
  } catch (err) {
    console.error("🔥 FATAL TRAIN sendFare Error:", {
      bookingId: booking?.id,
      error: err.message,
    });

    const userPhone = booking ? getUserPhone(booking) : null;

    if (userPhone) {
      await sendText(
        userPhone,
        "⚠️ Something went wrong while processing fare.",
      );
    }

    return false;
  }
};
