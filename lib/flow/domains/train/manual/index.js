...
/* ======================================================
   Normalize phone format (prevents +91 mismatch bugs)
====================================================== */
...
module.exports = async function handleTrainManual(ctx, text) {
  try {
    ...
    /* ======================================================
       🖼 IMAGE FORWARDING (SAFE + EARLY EXIT)
    ====================================================== */
    if (ctx.msg?.type === "image") {
      ...
      if (!bookingId) {
        await sendText(
          admin,
          "❌ No booking selected. Use PROCESS <BOOKING_ID>"
        );
        return true;
      }
      ...
      try {
        ...
        await sendImage(booking.user, imageId, caption);
        ...
        // 🔥 STOP processing caption as command
        return true;
      } catch (err) {
        console.error("🔥 Image forward failed:", err);
        await sendText(admin, "❌ Failed to forward image.");
        return true;
      }
    }
...
PG 12`
    ...
    if (ctx.session?.state === STATES.AWAITING_FARE_INPUT) {
      ...
      await sendText(
        admin,
        "✅ Fare sent to user successfully."
      );
