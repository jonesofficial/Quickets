...
/* ======================================================
   Determine correct user phone
====================================================== */
function getUserPhone(booking) {
  const userPhone = booking.user || booking.phone;
  return normalize(userPhone);
}

/* ======================================================
   Parse WL / RAC / AVAILABLE from screenshot caption
====================================================== */
function parseAvailabilityFromCaption(caption) {
  if (!caption) return null;
...
module.exports = async function handleTrainManual(ctx, text) {
  try {
    ...
    if (ctx.msg?.type === "image") {
      let bookingId = ctx.session?.bookingId;

      if (!bookingId && ctx.msg?.image?.caption) {
        const match = ctx.msg.image.caption.match(/BOOKING ID:\s*(\S+)/i);
    ...
    console.log("🚆 TRAIN ADMIN INPUT:", message);
    console.log("📦 Current bookingId:", ctx.session?.bookingId);
...
You may also send a screenshot.`,
    ...
    if (statusMatch) {
      ...
      updateBooking(booking.id, {
        availability: statusText,
        status: BOOKING_STATUS.AVAILABAILITY_SHARED,
      });
