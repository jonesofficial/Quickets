  ...
  try {
    ...
    if (ctx.msg?.type === "image") {
      ...
      try {
        ...
        /* Inject booking id if caption contains N/A */
        ...
        console.log("🚆 Sending train image to user:", ctx.session.bookingUser);

        await sendImage(
          ctx.session.bookingUser,
          imageId,
          caption
        );
        ...
        await sendText(
          admin,
          "❌ Failed to forward availability image."
        );
