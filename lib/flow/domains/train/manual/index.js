...
module.exports = async function handleTrainManual(ctx, text) {
  try {
    ...
    if (ctx.msg?.type === "image") {
      ...
      const userPhone = normalize(
        booking.user || booking.phone || booking.from,
      );
      ...
      try {
        ...
        if (availability) {
          ...
          console.log("🚆 Availability detected:", availability);
          await sendAvailabilityPrompt(userPhone, availability.type, booking);
        }
...
You may also send a screenshot.`,
    ...
    if (statusMatch) {
      ...
      if (booking.availability) {
        console.log("🚆 Availability already detected from screenshot");
        return true;
      }
      ...
      await sendAvailabilityPrompt(userPhone, statusText, booking);
