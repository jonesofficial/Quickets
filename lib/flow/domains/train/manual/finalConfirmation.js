...
//       ctx.msg?.interactive?.list_reply?.id;
...
async function handleFinalConfirmation(ctx) {
  try {
    ...
    /* ensure session has bookingId */
    if (!ctx.session.bookingId) {
      ctx.session.bookingId = booking.id;
    }

    /* ======================================================
       AVAILABLE → SEAT WARNING
    ====================================================== */

    if (buttonId === "TRAIN_BOOK_NOW") {
      ctx.session.state = TRAIN_MANUAL_STATES.AWAITING_AVAILABILITY_DECISION;
...
Do you want to continue?`,
    ...
    if (buttonId === "CONFIRM_BOOKING") {
      ...
      updateBooking(booking.id, {
        status: BOOKING_STATUS.AWAITING_FARE,
      });
      await sendText(
        ctx.from,
        `✅ *Booking Confirmed*
...
PG 15`,
    ...
    if (buttonId === "CANCEL_BOOKING") {
      updateBooking(booking.id, { status: BOOKING_STATUS.CANCELLED });
