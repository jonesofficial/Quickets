    ...
    if (s.state === "TRAIN_PASSENGERS" && interactiveType === "list_reply") {
      ...
      if (s.pendingBooking.quickMode) {
        ...
        const savedBooking = saveBooking({
          ...
          trainName: s.pendingBooking.trainName,
          departureTime: s.pendingBooking.departureTime,

          berth: s.pendingBooking.berthPref,
...
Please check availability and send screenshot.`,
          );
        }

        await sendText(
          from,
          "✅ Request received. Our agent is checking availability.",
        );

        /* 🔥 Important: hand control to passengerFlow */
        s.state = "PASSENGER_DETAILS";
