...
async function resolveStationsDynamic(query) {
  ...
  // 1️⃣ Search stations matching the query (Chennai, Bangalore, etc)
  const search = await stationSearch(query, 10);
  if (!search.length) return [];

  // 2️⃣ Internally choose a city-level reference station
  // (User does NOT select this)
  const reference =
    [...search]
      .filter((s) => s.lat && s.lon)
      .sort((a, b) => {
        // Prefer names that contain the searched text
  ...
  // 3️⃣ Fetch nearby stations around the searched city
  if (reference) {
  ...
  // 4️⃣ Add all search-matching stations
  results.push(...search);
  ...
  // 6️⃣ Final list shown to user (single message
