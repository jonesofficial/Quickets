...
/* ======================================================
 * 🔤 Transliteration helper (NEW)
 * ====================================================== */
function normalizeToEnglish(input, lang) {
  if (!input) return input;
  if (lang === "en") return input;

  try {
    if (lang === "ta") {
      return Sanscript.t(input, "tamil", "iast");
    }

    if (lang === "hi") {
      return Sanscript.t(input, "devanagari", "iast");
    }
...
async function resolveStations(query, lang) {
  ...
  const normalized = normalizeToEnglish(query.trim(), lang);

  // 🔍 Debug (remove later)
  console.log("STATION SEARCH:", query, "→", normalized);

  return await stationSearch(normalized, 6);
}
