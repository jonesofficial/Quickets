...
module.exports = async function handleTrainSelection(ctx, text) {
  ...
  if (!booking || !booking.availableTrains) {
    await sendText(ctx.from, "❌ Train list expired. Please search again.");
  ...
  if (!train) {
    await sendText(ctx.from, "❌ Invalid train selection.");
...
🎟️ *T${index + 1}* • ${train.trainNo} *${train.trainName}*
━━━━━━━━━━━━━━━━━━
...
⏳ Checking seat availability...`,
  ...
  ctx.session.state = TRAIN_STATES.AVAILABILITY_SCREENSHOT_PENDING;
