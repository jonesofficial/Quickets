...
module.exports = async function busBookingFlow(ctx) {
  ...
  try {
    ...
    const upperText = msg.type === "text" ? text?.trim().toUpperCase() : null;
    ...
    if (upperText === "BOOK AGAIN") {
      ...
      return false; // let menuFlow / index handle fresh start
    }
    ...
    if (s.state === "BUS_BUDGET" && interactiveType === "list_reply") {
      ...
      s.state = "PAX_MODE"; // 🔑 HANDOVER TO passengerFlow
