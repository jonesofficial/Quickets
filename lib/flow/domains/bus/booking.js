...
/* ======================================================
 * STANDARD PICKER (IMPORTANT)
 * ====================================================== */
function pickOption(id, get, lang = "en") {
  const label = get(id);

  if (!label) {
    console.warn(
      `⚠️ [Quickets] Missing label for option "${id}" (lang: ${lang})`
    );
  }

  return {
    id,
    label: label || id, // safe fallback
  };
}
...
/* ======================================================
 * Validator (FINAL GUARD)
 * ====================================================== */
function validateBusBooking(b) {
  return Boolean(
    b &&
      b.from &&
      b.to &&
...
module.exports = async function busBookingFlow(ctx) {
  ...
  try {
    ...
    // 🚨 GUARD: prevent skipping time selection
    if (s.state === "BUS_TIME" && interactiveType !== "list_reply") {
      await resendCurrentPrompt(ctx);
      return true;
    }
    ...
    if (s.state === "PAX_MODE" && interactiveType === "button_reply") {
      ...
      if (interactiveId === "PAX_BULK") {
        ...
        s.state = "PAX_BULK_INPUT";

        await sendText(
          from,
          get("FILL_PAX_BULK").replace("{{total}}", s.pendingBooking.paxCount)
        );
        return true;
      }
      ...
      if (interactiveId === "PAX_ONEBYONE") {
        ...
        await sendText(
          ...
          get("ENTER_NAME_PROMPT")
            .replace("{{i}}", 1)
            .replace("{{total}}", s.pendingBooking.paxCount)
        );
        return true;
      }

      // 🚨 Any other button → re-prompt
      await resendCurrentPrompt(ctx);
      return true;
    }
    ...
    if (s.state === "BOOKING_REVIEW" && interactiveId === "CONFIRM_BOOKING") {
      if (!validateBusBooking(s.pendingBooking)) {
        await sendText(
          from,
      ...
      // 🔴 CRITICAL: RESET STATE FIRST (prevents loop)
      s.pendingBooking = null;
      s.state = null;

      const saved = saveBooking(finalBooking);
