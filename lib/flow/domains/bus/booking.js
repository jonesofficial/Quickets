...
const handleBusMode = require("./modeSelector");
...
module.exports = async function busBookingFlow(ctx) {
  ...
  try {
    ...
    // 🔥 Ignore non-user message events
    const isUserMessage =
      msg?.type === "text" ||
    ...
    if (!isUserMessage) {
      return true; // Do nothing silently
    }

    if (msg?.type === "text" && text?.toLowerCase().includes("seat")) {
    ...
    const modeResult = await handleBusMode(ctx);
    ...
    if (s.state?.startsWith("BUS_QUICK_BOOK")) {
      return handleQuickBook(ctx);
    }
    /* ========== CITY CONFIRMATION ========== */
    if (
...
=============================== */
    ...
    if (s.state === "BUS_DATE") {
      ...
      if (
        ...
        if (interactiveId === "DATE_MANUAL") {
          await sendText(
            from,
            "📅 Please enter your journey date in format:\n\nDD-MM-YYYY\n\nExample: 25-02-2026",
          );
          return true;
        }

        if (s.pendingBooking.date) {
          s.state = "BUS_TIME";
          await resendCurrentPrompt(ctx);
          return true;
        }
      ...
      /* ---------- MANUAL DATE INPUT ---------- */
      if (msg?.type === "text") {
        const manualDate = parseManualDate(text || msg.text?.body);
        ...
        s.pendingBooking.date = formatDate(manualDate);
        s.state = "BUS_TIME";
    ...
    if (s.state === "BUS_PAX_COUNT" && interactiveType === "list_reply") {
      ...
      // 🔥 If QuickBook, skip seat type & budget
