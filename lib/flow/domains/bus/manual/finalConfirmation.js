...
async function buildFinalSummary(ctx) {
  try {
    ...
    lines.push("━━━━━━━━━━━━━━━━━━━━━━");
    lines.push(
      "⚠️ *Please confirm that all details are correct.*"
    );
    lines.push(
      "Once confirmed, we will fetch the final ticket price."
    );
    lines.push("");
  ...
  try {
    ...
    if (buttonId === "CONFIRM_BOOKING") {
      try {
        ...
        console.error(
          "🔥 Failed updating booking status on confirm",
          {
...
━━━━━━━━━━━━━━━━━━
👉 NEXT STEP:
Wait for updated seat selection.`
        );
      }
    ...
    try {
      ...
      console.error(
        "🔥 Failed sending final confirmation buttons",
        {
          bookingId: booking.id,
          error: err.message,
        }
      ...
      await sendText(ctx.from, summaryText);
    }

    return true;
  } catch (err) {
    console.error(
      "🔥 FATAL handleFinalConfirmation error",
      {
        user: ctx?.from,
