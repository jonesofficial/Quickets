...
async function handleFinalConfirmation(ctx) {
  ...
  if (ctx.session.state === BUS_STATES.FARE_PENDING) {
    await sendText(ctx.user, "⏳ Price confirmation already in progress.");
    return;
  }
  ...
  if (text === "2") {
    ...
    await sendText(ctx.user, "✏️ You can now edit your booking details.");

    return;
  }

  await sendText(ctx.user, "Please reply with:\n1️⃣ Confirm\n2️⃣ Edit Details");
}
