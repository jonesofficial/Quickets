...
⏰ ${selected.time}`,
  ...
  // Move to final confirmation waiting state
  session.state = BUS_STATES.FINAL_CONFIRMATION;

  await sendText(
    from,
    `📝 *Boarding & Dropping Updated Successfully!*\n\n` +
      `📍 Boarding: *${session.selectedBoardingPoint?.location || "-"}*\n` +
      `📍 Dropping: *${session.selectedDroppingPoint?.location || "-"}*\n\n` +
      `━━━━━━━━━━━━━━━━━━\n` +
      `👉 Please type *CONFIRM* to continue and view your final booking summary.`,
  );
