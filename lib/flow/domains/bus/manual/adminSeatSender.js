...
async function handleAdminSeatSender(ctx, text) {
  ...
  if (ctx.session.seatSelectionActive) {
    await sendText(from, "⚠️ Seat selection already active for this booking.");
    return true;
  }
  ...
  if (typeof image !== "string") {
    await sendText(from, "❌ Invalid seat image. Please resend.");
    return true;
  }
   ...
   * SEND IMAGE TO USER
   * ========================= */
  await sendImage(ctx.session.bookingUser, image);
   ...
   * CONFIRM TO ADMIN
   * ========================= */
  await sendText(from, "✅ Seat layout & options sent to user.\n\n" + seatText);
