...
async function handleAdminSeatSender(ctx, text) {
  ...
  try {
    ...
    if (!booking || booking.type !== "BUS") {
      ...
      await sendText(from, "❌ Seat layout is only valid for BUS bookings.");
      return true;
    }
    ...
    const image = ctx.msg?.image?.id || ctx.msg?.image?.link || null;
    ...
    if (!image) {
      ...
      await sendText(from, "❌ Could not extract seat image. Please resend.");
      return true;
    }
    ...
    if (/^SEAT_SELECTED/i.test(text || "")) {
      try {
        ...
        await sendText(userPhone, "✅ The seat has been selected.");
       ...
       ━━━━━━━━━━━━━━━━━━
        ...
        After boarding selection,
        send dropping points using:
    ...
    try {
      await sendSeatLayout({ session: userSession }, image);
    } catch (err) {
    ...
    await sendText(from, "❌ Internal error while processing seat layout.");
