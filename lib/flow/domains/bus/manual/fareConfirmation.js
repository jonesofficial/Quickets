...
//      // 🚫 Only handle buttons when user is at fare confirmation stage
//     if (ctx.session?.state !== BUS_STATES.FARE_CONFIRMATION) {
//       return false;
//     }
...
//     if (!booking) {
//       await sendText(ctx.from, "⚠️ No active booking found in fare confirmation.");
//       return true;
//     }
...
module.exports = async function handleFareConfirmation(ctx) {
  try {
    const buttonId =
      ctx.interactiveId || ctx.msg?.interactive?.button_reply?.id;

    if (!buttonId) return false;

    /* ===============================================
       Only run during FARE_CONFIRMATION
    =============================================== */
    ...
    if (!booking) {
      await sendText(
        ctx.from,
        "⚠️ No active booking found in fare confirmation.",
      );
      return true;
    }
    ...
    if (buttonId === "PROCEED_PAYMENT") {
      ...
      try {
        ...
        if (!payment || !payment.link) {
          console.error("❌ Payment generation failed", booking.id);
