// // lib/flow/index.js
// require("dotenv").config();

// const { buildContext } = require("./context");
// const { isRetry, handleRetry } = require("./retry");
// const { isBack, handleBack } = require("./back");
// /* ======================================================
//  * CORE FLOWS
//  * ====================================================== */
// const languageFlow = require("./languageFlow");
// const menuFlow = require("./menuFlow");
// const bookingFlow = require("./bookingFlow");
// const passengerFlow = require("./passengerFlow");
// const summaryFlow = require("./summaryFlow");
// const trackingFlow = require("./trackingFlow");
// const paymentFlow = require("./paymentFlow");
// const fallbackFlow = require("./fallbackFlow");
// const { handleDebug } = require("../dev/debugController");

// /* ======================================================
//  * BUS MANUAL FLOW HANDLERS
//  * ====================================================== */
// const handleBusSelection = require("./domains/bus/manual/busSelection");
// const handleSeatSelection = require("./domains/bus/manual/seatSelection");
// const handleSeatConfirmation = require("./domains/bus/manual/seatConfirmation");
// const BUS_STATES = require("./domains/bus/manual/states");
// const {
//   handleBoardingSelection,
//   handleDroppingSelection,
// } = require("./domains/bus/manual/boardingDropping");
// const {
//   handleFinalConfirmation,
// } = require("./domains/bus/manual/finalConfirmation");

// /* ======================================================
//  * BOOKING STORE
//  * ====================================================== */
// const {
//   getLastBookingByUser,
//   findBookingById,
//   sendBookingStatus,
// } = require("../bookingStore");
// const normalizeInput = require("../utils/inputTranslator");

// const { sendText } = require("../waClient");

// /* ======================================================
//  * ADMIN & SESSION
//  * ====================================================== */
// const { handleAdminCommands } = require("../admin/index");
// const { startOrGet, isProcessed, markProcessed } = require("../sessionStore");

// /* ======================================================
//  * ADMIN CONFIG
//  * ====================================================== */
// const RAW_ADMIN = process.env.ADMIN_PHONE || process.env.ADMIN_NUMBER;

// function normalize(num = "") {
//   return String(num).replace(/\D/g, "");
// }

// function isAdminNumber(from) {
//   if (!RAW_ADMIN) return false;
//   return normalize(from) === normalize(RAW_ADMIN);
// }

// /* ======================================================
//  * MAIN ROUTER
//  * ====================================================== */
// module.exports = async function route(req, res) {
//   const messages = req.body?.entry?.[0]?.changes?.[0]?.value?.messages;

// if (!messages) return;

// for (const msg of messages) {

//   if (!msg || !msg.from || !msg.id) continue;

//   const from = msg.from;

//   /* ================= DUPLICATE PROTECTION ================= */
//   if (isProcessed(msg.id)) return;
//   markProcessed(msg.id);

//   /* ======================================================
//    * ================= ADMIN FLOW =================
//    * ====================================================== */
//   if (isAdminNumber(from)) {
//     const { session } = startOrGet(from);
//     session.__isAdmin = true;

//     const ctx = buildContext(req, session);
//     if (!ctx) return;

//     if (msg.type === "text") {
//       const text = msg.text?.body?.trim().toUpperCase();

//       if (text === "STATUS") {
//         const booking = getLastBookingByUser(from);
//         if (!booking) {
//           await sendText(from, "❌ No recent booking found.");
//           return;
//         }
//         await sendBookingStatus(booking);
//         return;
//       }

//       if (text.startsWith("STATUS ")) {
//         const id = text.split(" ")[1]?.trim();
//         const booking = findBookingById(id);
//         if (!booking) {
//           await sendText(from, "❌ Booking not found.");
//           return;
//         }
//         await sendBookingStatus(booking);
//         return;
//       }
//     }

//     /* ======================================================
//      * ADMIN COMMAND ROUTING
//      * ====================================================== */

//     const handled = await handleAdminCommands(ctx);

//     // If nothing handled the admin message
//     if (!handled) {
//       // Allow image forwarding errors to show clearer message
//       if (msg.type === "image") {
//         await sendText(
//           from,
//           "⚠️ No booking selected.\nUse: PROCESS <BOOKING_ID> before sending availability image.",
//         );
//         return;
//       }

//       await sendText(
//         from,
//         "⚠️ Unknown admin command.\nSend *HELP* to see valid commands.",
//       );
//     }

//     return;
//   }

//   /* ======================================================
//    * ================= USER FLOW =================
//    * ====================================================== */
//   const { session } = startOrGet(from);
//   const ctx = buildContext(req, session);
//   if (!ctx) return;

//   /* ================= INPUT NORMALIZATION ================= */
//   await normalizeInput(ctx);

//   /* ================= GLOBAL RETRY ================= */
//   if (isRetry(ctx.text)) {
//     return handleRetry(ctx);
//   }

//   /* ================= GLOBAL BACK ================= */
//   if (isBack(ctx.text)) {
//     const handled = await handleBack(ctx);
//     if (handled) {
//       // 🔥 Clear user input
//       ctx.text = null;

//       // 🔥 Now continue flow execution from this point
//     } else {
//       return;
//     }
//   }

//   /* ================= GLOBAL COMMANDS ================= */
//   if (msg.type === "text") {
//     const text = msg.text?.body?.trim().toUpperCase();

//     if (text === "STATUS") {
//       const booking = getLastBookingByUser(from);
//       if (!booking) {
//         await sendText(from, "❌ No active booking found.");
//         return;
//       }
//       await sendBookingStatus(booking);
//       return;
//     }

//     if (text === "BOOK AGAIN") {
//       session.state = null;
//       session.busOptions = null;
//       session.tempSelectedBus = null;
//       session.selectedBus = null;
//       session.selectedSeat = null;
//       session.seatSelectionActive = false;

//       await sendText(from, "🔄 Starting a new booking...");
//       await menuFlow(ctx);
//       return;
//     }
//   }

//   if (msg.type === "text") {
//     const text = msg.text?.body?.trim().toUpperCase();

//     /* ================= START COMMAND ================= */

//     if (text === "START") {
//       // 🔄 Reset entire session
//       Object.keys(session).forEach((k) => delete session[k]);

//       // start fresh state
//       session.state = null;

//       await sendText(from, "🔄 Restarting Quickets...");

//       // send language selection again
//       await languageFlow(ctx);

//       return;
//     }

//     if (text === "STATUS") {
//       const booking = getLastBookingByUser(from);
//       if (!booking) {
//         await sendText(from, "❌ No active booking found.");
//         return;
//       }
//       await sendBookingStatus(booking);
//       return;
//     }
//   }

//   /* ======================================================
//    * 🔒 BUS OPTION SELECTION (STRICT STATE BASED)
//    * ====================================================== */
//   if (session.state === BUS_STATES.BUS_OPTION_SELECTION) {
//     if (
//       Array.isArray(session.busOptions) &&
//       session.busOptions.length > 0 &&
//       msg.type === "text" &&
//       /^\d+$/.test(msg.text?.body)
//     ) {
//       await handleBusSelection(ctx);
//       return;
//     }

//     await sendText(
//       from,
//       "🚌 Please select a bus by replying with the *bus number* shown above.",
//     );
//     return;
//   }

//   /* ======================================================
//    * 🔒 SEAT CONFIRMATION / FINAL CONFIRM
//    * ====================================================== */
//   if (
//     session.state === BUS_STATES.SEAT_CONFIRMATION ||
//     session.state === BUS_STATES.FINAL_CONFIRM
//   ) {
//     await handleSeatConfirmation(ctx);
//     return;
//   }

//   /* ======================================================
//    * 🔒 SEAT SELECTION
//    * ====================================================== */
//   if (
//     session.state === BUS_STATES.SEAT_SELECTION &&
//     session.seatSelectionActive &&
//     !session.selectedSeat
//   ) {
//     if (msg.type === "text") {
//       await handleSeatSelection(ctx);
//       return;
//     }

//     await sendText(
//       from,
//       "🪑 Please select a seat by replying with the *seat number* shown in the layout.",
//     );
//     return;
//   }

//   /* ======================================================
//    * 🔒 BOARDING / DROPPING
//    * ====================================================== */
//   if (await handleBoardingSelection(ctx)) return;
//   if (await handleDroppingSelection(ctx)) return;

//   /* ======================================================
//    * 🔒 FINAL CONFIRMATION (after boarding)
//    * ====================================================== */
//   if (session.state === BUS_STATES.FINAL_CONFIRMATION) {
//     await handleFinalConfirmation(ctx);
//     return;
//   }

//   /* ======================================================
//    * 🔒 FARE CONFIRMATION
//    * ====================================================== */
//   {
//     const handleFareConfirmation = require("./domains/bus/manual/fareConfirmation");
//     if (await handleFareConfirmation(ctx)) return;
//   }

//   /* ======================================================
//    * 🔒 PAYMENT FLOW
//    * ====================================================== */
//   if (await paymentFlow(ctx)) return;

//   /* ======================================================
//    * 🌐 NORMAL FLOW
//    * ====================================================== */
//   if (await handleDebug(ctx)) return;
//   if (await languageFlow(ctx)) return;
//   if (await menuFlow(ctx)) return;
//   if (await passengerFlow(ctx)) return;
//   if (await summaryFlow(ctx)) return;
//   if (await bookingFlow(ctx)) return;
//   if (await trackingFlow(ctx)) return;

//   /* ======================================================
//    * 🤖 FALLBACk
//    * ====================================================== */
//   await fallbackFlow(ctx);
// }
// res.sendStatus(200);
// };

// lib/flow/index.js
require("dotenv").config();

const { buildContext } = require("./context");
const { isRetry, handleRetry } = require("./retry");
const { isBack, handleBack } = require("./back");

/* ======================================================
 * CORE FLOWS
 * ====================================================== */
const languageFlow = require("./languageFlow");
const menuFlow = require("./menuFlow");
const bookingFlow = require("./bookingFlow");
const passengerFlow = require("./passengerFlow");
const summaryFlow = require("./summaryFlow");
const trackingFlow = require("./trackingFlow");
const paymentFlow = require("./paymentFlow");
const fallbackFlow = require("./fallbackFlow");
const { handleDebug } = require("../dev/debugController");

/* ======================================================
 * BUS MANUAL FLOW HANDLERS
 * ====================================================== */
const handleBusSelection = require("./domains/bus/manual/busSelection");
const handleSeatSelection = require("./domains/bus/manual/seatSelection");
const handleSeatConfirmation = require("./domains/bus/manual/seatConfirmation");
const BUS_STATES = require("./domains/bus/manual/states");

const {
  handleBoardingSelection,
  handleDroppingSelection,
} = require("./domains/bus/manual/boardingDropping");

const {
  handleFinalConfirmation,
} = require("./domains/bus/manual/finalConfirmation");

/* ======================================================
 * BOOKING STORE
 * ====================================================== */
const {
  getLastBookingByUser,
  findBookingById,
  sendBookingStatus,
} = require("../bookingStore");

const normalizeInput = require("../utils/inputTranslator");
const { sendText } = require("../waClient");

/* ======================================================
 * ADMIN & SESSION
 * ====================================================== */
const { handleAdminCommands } = require("../admin/index");
const { startOrGet, isProcessed, markProcessed } = require("../sessionStore");

/* ======================================================
 * ADMIN CONFIG
 * ====================================================== */
const RAW_ADMIN = process.env.ADMIN_PHONE || process.env.ADMIN_NUMBER;

function normalize(num = "") {
  return String(num).replace(/\D/g, "");
}

function isAdminNumber(from) {
  if (!RAW_ADMIN) return false;
  return normalize(from) === normalize(RAW_ADMIN);
}

/* ======================================================
 * MAIN ROUTER
 * ====================================================== */
module.exports = async function route(req, res) {
  const messages = req.body?.entry?.[0]?.changes?.[0]?.value?.messages;
  if (!messages) return;

  for (const msg of messages) {
    if (!msg || !msg.from || !msg.id) continue;

    const from = msg.from;

    /* ================= DUPLICATE PROTECTION ================= */
    if (isProcessed(msg.id)) continue;
    markProcessed(msg.id);

    /* ======================================================
     * ================= ADMIN FLOW =================
     * ====================================================== */
    if (isAdminNumber(from)) {
      const { session } = startOrGet(from);
      session.__isAdmin = true;

      const ctx = buildContext(req, msg, session);
      if (!ctx) continue;

      if (msg.type === "text") {
        const text = msg.text?.body?.trim().toUpperCase();

        if (text === "STATUS") {
          const booking = getLastBookingByUser(from);

          if (!booking) {
            await sendText(from, "❌ No recent booking found.");
            continue;
          }

          await sendBookingStatus(booking);
          continue;
        }

        if (text.startsWith("STATUS ")) {
          const id = text.split(" ")[1]?.trim();
          const booking = findBookingById(id);

          if (!booking) {
            await sendText(from, "❌ Booking not found.");
            continue;
          }

          await sendBookingStatus(booking);
          continue;
        }
      }

      const handled = await handleAdminCommands(ctx);

      if (!handled) {
        if (msg.type === "image") {
          await sendText(
            from,
            "⚠️ No booking selected.\nUse: PROCESS <BOOKING_ID> before sending availability image.",
          );
          continue;
        }

        await sendText(
          from,
          "⚠️ Unknown admin command.\nSend *HELP* to see valid commands.",
        );
      }

      continue;
    }

    /* ======================================================
     * ================= USER FLOW =================
     * ====================================================== */

    const { session } = startOrGet(from);
    const ctx = buildContext(req, msg, session);

    if (!ctx) continue;

    /* ================= INPUT NORMALIZATION ================= */
    await normalizeInput(ctx);

    /* ================= GLOBAL RETRY ================= */
    if (isRetry(ctx.text)) {
      await handleRetry(ctx);
      continue;
    }

    /* ================= GLOBAL BACK ================= */
    if (isBack(ctx.text)) {
      const handled = await handleBack(ctx);

      if (handled) {
        ctx.text = null;
      } else {
        continue;
      }
    }

    /* ================= GLOBAL COMMANDS ================= */

    if (msg.type === "text") {
      const text = msg.text?.body?.trim().toUpperCase();

      if (text === "START") {
        Object.keys(session).forEach((k) => delete session[k]);
        session.state = null;

        await sendText(from, "🔄 Restarting Quickets...");
        await languageFlow(ctx);

        continue;
      }

      // if (text === "STATUS") {

      //   const booking = getLastBookingByUser(from);

      //   if (!booking) {
      //     await sendText(from, "❌ No active booking found.");
      //     continue;
      //   }

      //   await sendBookingStatus(booking);
      //   continue;
      // }

      if (text === "BOOK AGAIN") {
        session.state = null;
        session.busOptions = null;
        session.tempSelectedBus = null;
        session.selectedBus = null;
        session.selectedSeat = null;
        session.seatSelectionActive = false;

        await sendText(from, "🔄 Starting a new booking...");
        await menuFlow(ctx);

        continue;
      }
    }

    /* ======================================================
     * 🔒 BUS OPTION SELECTION
     * ====================================================== */

    if (session.state === BUS_STATES.BUS_OPTION_SELECTION) {
      if (
        Array.isArray(session.busOptions) &&
        session.busOptions.length > 0 &&
        msg.type === "text" &&
        /^\d+$/.test(msg.text?.body)
      ) {
        await handleBusSelection(ctx);
        continue;
      }

      await sendText(
        from,
        "🚌 Please select a bus by replying with the *bus number* shown above.",
      );

      continue;
    }

    /* ======================================================
     * 🔒 SEAT CONFIRMATION
     * ====================================================== */

    if (
      session.state === BUS_STATES.SEAT_CONFIRMATION ||
      session.state === BUS_STATES.FINAL_CONFIRM
    ) {
      await handleSeatConfirmation(ctx);
      continue;
    }

    /* ======================================================
     * 🔒 SEAT SELECTION
     * ====================================================== */

    if (
      session.state === BUS_STATES.SEAT_SELECTION &&
      session.seatSelectionActive &&
      !session.selectedSeat
    ) {
      if (msg.type === "text") {
        await handleSeatSelection(ctx);
        continue;
      }

      await sendText(
        from,
        "🪑 Please select a seat by replying with the *seat number* shown in the layout.",
      );

      continue;
    }

    /* ======================================================
     * 🔒 BOARDING / DROPPING
     * ====================================================== */

    if (await handleBoardingSelection(ctx)) continue;
    if (await handleDroppingSelection(ctx)) continue;

    /* ======================================================
     * 🔒 FINAL CONFIRMATION
     * ====================================================== */

    if (session.state === BUS_STATES.FINAL_CONFIRMATION) {
      await handleFinalConfirmation(ctx);
      continue;
    }

    /* ======================================================
     * 🔒 FARE CONFIRMATION
     * ====================================================== */

    const handleFareConfirmation = require("./domains/bus/manual/fareConfirmation");
    if (await handleFareConfirmation(ctx)) continue;


    const handleTrainFare = require("./domains/train/manual/fareFlow");
    if (await handleTrainFare(ctx)) continue;

    if (await paymentFlow(ctx)) continue;
    /* ======================================================
     * 🔒 PAYMENT FLOW
     * ====================================================== */

    if (await paymentFlow(ctx)) continue;

    /* ======================================================
     * 🌐 NORMAL FLOWS
     * ====================================================== */

    if (await handleDebug(ctx)) continue;
    if (await languageFlow(ctx)) continue;
    if (await menuFlow(ctx)) continue;
    if (await passengerFlow(ctx)) continue;
    if (await summaryFlow(ctx)) continue;
    if (await bookingFlow(ctx)) continue;
    if (await trackingFlow(ctx)) continue;

    /* ======================================================
     * 🤖 FALLBACK
     * ====================================================== */

    await fallbackFlow(ctx);
  }

  if (!res.headersSent) {
    res.sendStatus(200);
  }
};
