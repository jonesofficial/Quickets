...
module.exports = async function route(req, res) {
  ...
  /* ==================================================
   * 🚨 HARD ADMIN SHORT-CIRCUIT (BEFORE EVERYTHING)
   * ================================================== */
  if (msg.type === "text" && isAdmin(msg.from)) {
    const rawText = msg.text?.body?.trim();

    if (rawText?.startsWith("APPROVE") || rawText?.startsWith("PAID")) {
      console.log("🛂 RAW ADMIN COMMAND:", rawText);
