...
START – Restart bot
MENU – Main menu
STATUS – Latest booking
TRACK <ID> – Track booking
MY BOOKINGS – Your bookings
TICKET <ID> – Download ticket
...
CANCEL – Cancel current booking`,
    ...
    if (text === "MY BOOKINGS") {
      const bookings = findBookingsByUser(from);

      if (!bookings.length) {
        await sendText(from, "❌ You have no bookings yet.");
        return;
      }
      ...
      let msgText = "📦 *Your Recent Bookings*\n\n";
    ...
    if (text === "LANGUAGE") {
      ...
      await sendText(from, "🌍 Choose your language");
    ...
    if (text === "LAST BOOKING") {
      const booking = getLastBookingByUser(from);

      if (!booking) {
        await sendText(from, "❌ No previous booking found.");
        return;
      }
    ...
    if (text.startsWith("TICKET ")) {
      ...
      if (!booking.ticketDocumentId) {
        await sendText(from, "⚠️ Ticket not available yet.");
        return;
      }

      await sendDocumentById(
        from,
        booking.ticketDocumentId,
        `🎫 Your ticket for booking *${booking.id}*`,
      );
