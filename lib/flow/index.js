...
module.exports = async function route(req, res) {
  // lib/flow/index.js, inside module.exports = async function route(req, res) {
  console.log(
    "📩 FLOW RECEIVED MESSAGE ID:",
    msg?.id,
    "from:",
    msg?.from,
    "type:",
    msg?.type || "unknown",
    "interactiveType:",
    msg?.interactive?.type || msg?.type || "n/a",
  );
  ...
  if (isAdminNumber(from)) {
    ...
    const { session } = startOrGet(from);
    console.log("🧾 SESSION LOAD:", { from, session_snapshot: JSON.stringify(session, null, 2).slice(0, 2000) });
    session.__isAdmin = true;
  ...
  /* 🔥 CRITICAL FIX: BUS SELECTION FIRST */
  if (session.state === "BUS_OPTION_SELECTION") {
