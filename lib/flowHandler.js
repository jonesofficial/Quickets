...
//    // ===== declare interactive variables early so any branch can read them =====
//    let interactiveType = null;
//    let interactiveId = null;
//    if (msg.type === "interactive") {
//      interactiveType = msg.interactive?.type || null; // "button_reply" | "list_reply"
//      if (interactiveType === "button_reply") interactiveId = msg.interactive?.button_reply?.id || null;
//      if (interactiveType === "list_reply") interactiveId = msg.interactive?.list_reply?.id || null;
//    }
//
//
//    // -------------------------
...
//   // --- Robust language selection prompt & handler (REPLACE ORIGINAL BLOCK HERE) ---
//   if (!s.optionSet && s.state !== "LANG_SELECTION") {
//     s.state = "LANG_SELECTION";
//
//     // Build short safe button labels
//     const btnTitleEn = optionSets.en.LANG_EN_LABEL || "English";
//     const btnTitleTa = optionSets.en.LANG_TA_LABEL || "தமிழ்";
//
//     // debug
//     console.log("Showing language selection to", maskPhone(from));
//
//     // Try interactive buttons, but fall back to plain text if sendButtons fails
//     try {
//       await sendButtons(
//         from,
//         `${optionSets.en.WELCOME_TITLE}\n${optionSets.en.WELCOME_DESC}\n\n${optionSets.en.LANG_PROMPT}`,
//         [
//           { id: "LANG_EN", title: btnTitleEn },
//           { id: "LANG_TA", title: btnTitleTa },
//         ]
//       );
//       s.__langPromptMode = "buttons";
//       return res.sendStatus(200);
//     } catch (err) {
//       console.warn("sendButtons failed for language prompt:", err?.response?.data || err?.message || err);
//       // fallback: numbered text so user can reply "1" or "2"
//       await sendText(
//         from,
//         `${optionSets.en.WELCOME_TITLE}\n${optionSets.en.WELCOME_DESC}\n\n${optionSets.en.LANG_PROMPT}\n1. ${btnTitleEn}\n2. ${btnTitleTa}\n\nReply 1 or 2, or type English / தமிழ்`
//       );
//       s.__langPromptMode = "text";
//       return res.sendStatus(200);
//     }
//   }
//
//   // Accept typed fallbacks for language selection (if user replies with words or numbers)
//   if (!s.optionSet && s.state === "LANG_SELECTION" && msg.type === "text") {
//     const reply = textIn.toLowerCase().trim();
//
//     // typed/numbered mapping
//     if (["1", "en", "english"].includes(reply)) {
//       s.optionSet = "en";
//       s.state = "IDLE";
//       console.log("Language set (typed) to EN for", maskPhone(from));
//       await sendList(
//         from,
//         `${get("WELCOME_TITLE")}\n${get("WELCOME_DESC")}`,
//         get("MAIN"),
...
//     }
//
//     if (["2", "ta", "tamil", "தமிழ்"].includes(reply)) {
//       s.optionSet = "ta";
//       s.state = "IDLE";
//       console.log("Language set (typed) to TA for", maskPhone(from));
//       await sendList(
//         from,
//         `${get("WELCOME_TITLE")}\n${get("WELCOME_DESC")}`,
//         get("MAIN"),
...
//   }
//
//   // Handle interactive button replies for language (fall-through for button replies)
...
//     }
//
//     // fallback for unexpected interactive replies in this state
